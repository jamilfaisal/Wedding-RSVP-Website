...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.includes('/rsvp/edit') && request.nextUrl.searchParams.has('token')) {
    const token = request.cookies.get('wedding-auth')?.value;
    const referrer = request.headers.get('referer');
    const userAgent = request.headers.get('user-agent');

    console.log('RSVP Edit Request Debug:', {
      pathname,
    ...
    if (!authenticated) {
      console.log('Redirecting to auth-required page');
      const locale = getLocale(request);
      const authRequiredUrl = new URL(`/${locale}/auth-required`, request.url);
      authRequiredUrl.searchParams.set(
        'returnTo',
        request.nextUrl.pathname + request.nextUrl.search
      );
      return NextResponse.redirect(authRequiredUrl, { status: 302 });
    }
    // If authenticated, let the request continue without further auth checks
